...
double Buscador::DFR(int Doc) {
	double sim=0;
	double wiq=0,wid=0,ft=0,nt=0,ftd=0,freq=0,ld=0,avr_ld=0;
	...
	for(auto i=getIndicePregunta().begin();i!=getIndicePregunta().end();++i){//Para cada término de la pregunta
		...
		if(nt!=0){
			wid=((log10(1+(ft/getIndiceDocs().size()))/log10(2)) + (ftd*(log10((1+(ft/getIndiceDocs().size()))/getIndiceDocs().size())/log10(2)))) * ((ft+1)/(nt*(ftd+1)));
		}
		else{
			wid=0;
		}

		wiq=i->second.getFt()/getIndicePregunta().size();
		sim+=wiq+wid;
	}
	return sim;
}
